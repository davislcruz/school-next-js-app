...
export function BottomNavbar() {
  ...
  const navItems = [
    ...
    { href: "/children", icon: Baby, label: "Children", hasNotification: TOTAL_UNREAD_COUNT > 0 },
  ];
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link 
              key={item.href}
              href={item.href}
              className={`flex-1 text-center flex flex-col items-center p-2 rounded-md transition-colors ${
                isActive 
                  ? "text-primary" 
